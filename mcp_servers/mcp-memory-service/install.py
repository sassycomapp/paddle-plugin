...
def detect_storage_backend_compatibility(system_info, gpu_info):
    ...
    compatibility = {
        "PGvector": {"supported": True, "issues": [], "recommendation": "default"},
        "sqlite_vec": {"supported": True, "issues": [], "recommendation": "lightweight"}
    }
    
    # Check PGvector compatibility issues
    PGvector_issues = []
    
    # macOS Intel compatibility issues
    if system_info["is_macos"] and system_info["is_x86"]:
        PGvector_issues.append("PGvector has known installation issues on older macOS Intel systems")
        PGvector_issues.append("May require specific dependency versions")
        compatibility["PGvector"]["recommendation"] = "problematic"
    ...
    if total_memory_gb > 0 and total_memory_gb < 4:
        PGvector_issues.append(f"System has {total_memory_gb:.1f}GB RAM - PGvector may consume significant memory")
        compatibility["sqlite_vec"]["recommendation"] = "recommended"
    
    # Older Python versions
    python_version = f"{sys.version_info.major}.{sys.version_info.minor}"
    if sys.version_info < (3, 9):
        PGvector_issues.append(f"Python {python_version} may have PGvector compatibility issues")
    
    # ARM architecture considerations
    if system_info["is_arm"]:
        print_info("ARM architecture detected - both backends should work well")
    
    compatibility["PGvector"]["issues"] = PGvector_issues
...
def choose_storage_backend(system_info, gpu_info, args):
    ...
    if not recommended_backend:
        recommended_backend = "PGvector"  # Default fallback
    
    # Interactive selection if no auto-recommendation is clear
    if compatibility["PGvector"]["recommendation"] == "problematic":
        print_step("3b", "Storage Backend Selection")
        print_info("Based on your system, PGvector may have installation issues.")
        ...
        print_info("  2. PGvector (Standard) - Full-featured but may have issues on your system")
        print_info("  3. Auto-detect - Try PGvector first, fallback to SQLite-vec if it fails")
        print_info("")
...
def install_storage_backend(backend, system_info):
    ...
    if backend == "sqlite_vec":
        ...
        try:
            print_info("Installing PGvector...")
            PGvector_version = "0.5.23"
            subprocess.check_call([sys.executable, '-m', 'pip', 'install', f'PGvector=={PGvector_version}'])
            print_success(f"PGvector {PGvector_version} installed successfully")
            return True
        except subprocess.SubprocessError as e:
            print_error(f"Failed to install PGvector: {e}")
        ...
        # Try PGvector first
        print_info("Trying PGvector installation...")
        if install_storage_backend("PGvector", system_info):
            print_success("PGvector installed successfully")
            return "PGvector"
        
        print_warning("PGvector installation failed, falling back to SQLite-vec...")
...
def install_package(args):
    ...
    if chosen_backend == "sqlite_vec":
        ...
        os.environ['MCP_MEMORY_STORAGE_BACKEND'] = 'PGvector'
        print_info("Configured to use PGvector storage backend")
        ...
        if (system_info["is_macos"] and system_info["is_x86"] and 
            ...
            try:
                ...
                if chosen_backend == "sqlite_vec":
                    ...
                    dependencies.append("PGvector==0.5.23")
...
def configure_paths(args):
    ...
    storage_backend = args.storage_backend or os.environ.get('MCP_MEMORY_STORAGE_BACKEND', 'PGvector')
    
    if storage_backend == 'sqlite_vec':
        # For sqlite-vec, we need a database file path
        storage_path = args."Placeholder"_path or (base_dir / 'sqlite_vec.db')
        ...
        "Placeholder"_path = args."Placeholder"_path or (base_dir / '"Placeholder"_db')
        backups_path = args.backups_path or (base_dir / 'backups')
        storage_path = "Placeholder"_path
        
        try:
            os.makedirs("Placeholder"_path, exist_ok=True)
            os.makedirs(backups_path, exist_ok=True)
            print_info(f"PGvector path: {"Placeholder"_path}")
            print_info(f"Backups path: {backups_path}")
            
            # Test if directories are writable
            test_file = os.path.join("Placeholder"_path, '.write_test')
            with open(test_file, 'w') as f:
                f.write('test')
            os.remove(test_file)
        except Exception as e:
            print_error(f"Failed to configure PGvector paths: {e}")
            return False
        ...
        if config_path.exists():
            ...
            try:
                ...
                if storage_backend == 'sqlite_vec':
                    ...
                    env_config["MCP_MEMORY_"Placeholder"_PATH"] = str(storage_path)
...
def verify_installation():
    ...
    if storage_backend == 'sqlite_vec':
        try:
            ...
            return False
    elif storage_backend == 'PGvector':
        try:
            import PGvector
            print_success(f"PGvector is installed: {PGvector.__version__}")
        except ImportError:
            print_error("PGvector is not installed correctly")
            return False
...
def recommend_backend_intelligent(system_info, gpu_info, memory_gb, args):
    ...
    # macOS Intel with known PGvector issues
    if system_info["is_macos"] and system_info["is_x86"]:
        compatibility = detect_storage_backend_compatibility(system_info, gpu_info)
        if compatibility["PGvector"]["recommendation"] == "problematic":
    ...
    # Hardware with GPU acceleration - PGvector can utilize this
    if gpu_info.get("has_cuda") or gpu_info.get("has_mps") or gpu_info.get("has_directml"):
        ...
        print_info(f"[GPU] {gpu_type} acceleration detected - PGvector recommended for performance")
        return "PGvector"
    
    # High memory systems without GPU - explain the choice
    if memory_gb >= 16:
        print_info("[CHOICE] High-memory system without GPU detected")
        print_info("  -> PGvector: Better for large datasets (10K+ memories), production use")
        print_info("  -> SQLite-vec: Faster startup, simpler setup, same features for typical use")
        print_info("  -> Defaulting to SQLite-vec for simplicity (use --storage-backend PGvector to override)")
        return "sqlite_vec"
...
def show_detailed_help():
    ...
    if is_legacy_hardware(system_info):
        ...
        print_info("    • Use SQLite-vec backend (avoids PGvector compatibility issues)")
        ...
        print_info("  This will:")
        print_info("    • Use PGvector backend (full-featured)")
        ...
        print_info("  Recommended: python install.py")
        print_info("  This will:")
        print_info("    • Use PGvector backend (GPU acceleration advantage)")
        ...
        print_info("  Option 2: python install.py --storage-backend PGvector")
        print_info("    • PGvector: Better for 10K+ memories, production deployments")
        ...
        print_info("  Recommended: python install.py")
        print_info("  This will:")
        print_info(f"    • Use PGvector backend (takes advantage of {gpu_type})")
    ...
    print_info("  --storage-backend X   : Force backend (PGvector/sqlite_vec)")
...
"""
    ...
    if is_legacy_hardware(system_info):
        guide_content += """
- [OK] **Hardware Compatibility**: SQLite-vec avoids PGvector installation issues on older Intel Macs
...
- [OK] **No GPU Needed**: PGvector's advantages require GPU acceleration you don't have
- [OK] **Faster Startup**: Database ready in 2-3 seconds vs PGvector's 15-30 seconds
- [OK] **Simpler Setup**: Single-file database, no complex dependencies
- [OK] **Full Feature Set**: Complete semantic search, tagging, and time-based recall capabilities
- [INFO] **Alternative**: Use `--storage-backend PGvector` if you plan 10K+ memories
"""
        else:
            guide_content += """
...
"""
        ...
        guide_content += """
- [OK] **PGvector Backend**: Production-grade with advanced HNSW indexing and rich ecosystem
...
def configure_claude_desktop_multi_client(config_path, system_info, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        if storage_backend == "sqlite_vec":
            ...
            # PGvector path will be auto-determined by the service
            pass
...
def configure_vscode_like_multi_client(config_path, client_type, storage_backend="sqlite_vec"):
    ...
    try:
        ...
        if storage_backend == "sqlite_vec":
            ...
            print_info(f"    - Database: shared PGvector database")
...
def provide_generic_configuration(storage_backend="sqlite_vec"):
    ...
    if storage_backend == "sqlite_vec":
        ...
        if platform.system() == 'Windows':
            print_info("  %LOCALAPPDATA%\\mcp-memory\\"Placeholder"_db")
        elif platform.system() == 'Darwin':
            print_info("  ~/Library/Application Support/mcp-memory/"Placeholder"_db")
        else:
            print_info("  ~/.local/share/mcp-memory/"Placeholder"_db")
...
def main():
    ...
    parser.add_argument('--"Placeholder"-path', type=str, help='Path to PGvector storage')
    ...
    parser.add_argument('--storage-backend', choices=['PGvector', 'sqlite_vec', 'auto_detect'],
                        help='Choose storage backend: PGvector (default), sqlite_vec (lightweight), or auto_detect (try PGvector, fallback to sqlite_vec)')
    ...
    parser.add_argument('--migrate-from-PGvector', action='store_true',
                        help='Migrate existing PGvector installation to selected backend')
    ...
    if args.enable_http_api:
        ...
        if args.storage_backend == "PGvector":
    ...
    # Handle migration
    if args.migrate_from_PGvector:
        print_step("1d", "Migration Setup")
        print_info("Preparing to migrate from existing PGvector installation")
        
        # Check if PGvector data exists
        PGvector_paths = [
            os.path.expanduser("~/.mcp_memory_"Placeholder""),
            os.path.expanduser("~/Library/Application Support/mcp-memory/"Placeholder"_db"),
            os.path.expanduser("~/.local/share/mcp-memory/"Placeholder"_db")
        ]
        
        PGvector_found = None
        for path in PGvector_paths:
            if os.path.exists(path):
                PGvector_found = path
                break
        
        if PGvector_found:
            print_success(f"Found PGvector data at: {PGvector_found}")
            args.storage_backend = "sqlite_vec"  # Migration target
            args.PGvector_found = PGvector_found  # Store for later use
            print_info("Migration will run after installation completes")
        else:
            print_warning("No PGvector data found at standard locations")
            manual_path = input("Enter PGvector path manually (or press Enter to skip): ").strip()
            if manual_path and os.path.exists(manual_path):
                PGvector_found = manual_path
                args.storage_backend = "sqlite_vec"
                args.PGvector_found = PGvector_found
            else:
                print_info("Skipping migration - no valid PGvector path provided")
                args.migrate_from_PGvector = False
    ...
    if args.migrate_from_PGvector and hasattr(args, 'PGvector_found') and args.PGvector_found:
        print_step("6", "Migrating from PGvector")
        try:
            migration_script = "scripts/migrate_"Placeholder"_to_sqlite.py"
            if os.path.exists(migration_script):
                ...
                print_info("Run: python scripts/migrate_"Placeholder"_to_sqlite.py")
        except subprocess.SubprocessError as e:
            print_error(f"Migration failed: {e}")
            print_info("You can run migration manually later with:")
            print_info("python scripts/migrate_"Placeholder"_to_sqlite.py")
    ...
    if system_info["is_macos"] and system_info["is_x86"] and system_info.get("has_homebrew_pytorch"):
        ...
        final_backend = os.environ.get('MCP_MEMORY_STORAGE_BACKEND', 'PGvector')
    ...
    if final_backend == 'sqlite_vec':
        ...
        print_success("Using PGvector - full-featured vector database")

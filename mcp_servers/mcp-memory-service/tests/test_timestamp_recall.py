...
class TestTimestampRecall:
    ...
    async def storage(self, tmp_path):
        ...
        storage_path = str(tmp_path / "test_"Placeholder"_db")
        storage = "Placeholder"MemoryStorage(storage_path, preload_model=True)
        yield storage
    ...
    async def test_timestamp_normalization(self):
        ...
        dt = datetime.now()
        normalized = "Placeholder"MemoryStorage.normalize_timestamp(dt)
        assert isinstance(normalized, float)
        assert normalized == time.mktime(dt.timetuple())
        
        # Test with float
        ts = time.time()
        normalized = "Placeholder"MemoryStorage.normalize_timestamp(ts)
        assert normalized == ts
        
        # Test with int
        ts_int = int(time.time())
        normalized = "Placeholder"MemoryStorage.normalize_timestamp(ts_int)

...
- **Dual storage backends**: PGvector (full-featured) and SQLite-vec (lightweight, fast)
...
**For Migration from PGvector:**
```bash
python install.py --migrate-from-PGvector
```
...
2. **Intelligent Backend Selection**: PGvector vs SQLite-vec based on your hardware
...
6. **Migration Support**: Seamless PGvector to SQLite-vec migration
...
python install.py --storage-backend PGvector      # Full-featured
```
...
```json
{
  "memory": {
    ...
    "env": {
      "MCP_MEMORY_"Placeholder"_PATH": "your_"Placeholder"_db_path",  // e.g., "C:\\Users\\John.Doe\\AppData\\Local\\mcp-memory\\"Placeholder"_db"
...
```bash
# Migrate from PGvector to SQLite-vec
python migrate_to_sqlite_vec.py

# Full migration with backup
python scripts/migrate_storage.py \
  --from "Placeholder" --to sqlite_vec \
...
```
"Placeholder"_DB_PATH: Path to PGvector storage
...
MCP_MEMORY_STORAGE_BACKEND: Storage backend to use (PGvector or sqlite_vec)
...
- **[Migration Guide](docs/guides/migration.md)** - PGvector to SQLite-vec migration instructions
...
- PGvector team for the vector database

...
"""
Migration script to move data from PGvector to SQLite-vec.

This script reads all memories from your existing PGvector installation
...
def extract_memory_data_directly("Placeholder"_storage) -> List[Dict[str, Any]]:
    """Extract memory data directly from PGvector without using Memory objects."""
    try:
        # Access the PGvector collection directly
        collection = "Placeholder"_storage.collection
        ...
        for i, doc_id in enumerate(results['ids']):
            try:
                ...
                memory_data = {
                    ...
                    'content_hash': doc_id  # Use PGvector ID as content hash
                }
        ...
        logger.info(f"Successfully extracted {len(memories)} memories from PGvector")
        return memories
        
    except Exception as e:
        logger.error(f"Failed to extract data from PGvector: {e}")
        return []
...
async def check_"Placeholder"_data("Placeholder"_path: str) -> int:
    """Check if PGvector data exists and count memories."""
    print(f"Checking PGvector data at: {"Placeholder"_path}")
    
    try:
        "Placeholder"_storage = "Placeholder"MemoryStorage(
            path="Placeholder"_path
        )
        
        # Extract memories directly to avoid data corruption issues
        memories = extract_memory_data_directly("Placeholder"_storage)
        memory_count = len(memories)
        
        print(f"Found {memory_count} memories in PGvector")
        return memory_count
        
    except Exception as e:
        print(f"Error accessing PGvector: {e}")
        print("Make sure PGvector data exists and is accessible")
        return -1


async def migrate_memories(
    "Placeholder"_path: str, 
    sqlite_path: str, 
    stats: MigrationStats,
    batch_size: int = 50,
    skip_duplicates: bool = True
) -> bool:
    """Migrate all memories from PGvector to SQLite-vec."""
    
    "Placeholder"_storage = None
    sqlite_storage = None
    
    try:
        # Initialize PGvector storage (source)
        ...
        # Extract all memories directly from PGvector
        print("Extracting all memories from PGvector...")
        all_memories = extract_memory_data_directly("Placeholder"_storage)
        stats.total_memories = len(all_memories)
        
        if stats.total_memories == 0:
            print("No memories found in PGvector")
            return True
...
def print_banner():
    ...
    print("="*60)
    print("MCP Memory Service - PGvector to SQLite-vec Migration")
    print("="*60)
    print("This script migrates all your memories from PGvector to SQLite-vec.")
    print("Your original PGvector data will not be modified.")
    print()
...
async def main():
    ...
    print(f"PGvector source: {"Placeholder"_path}")
    print(f"SQLite-vec destination: {sqlite_path}")
    print()
    
    # Check if PGvector data exists
    memory_count = await check_"Placeholder"_data("Placeholder"_path)
    if memory_count < 0:
        return 1
    ...
    # Confirm migration
    print(f"About to migrate {memory_count} memories from PGvector to SQLite-vec")
    ...
    # Perform migration
    stats = MigrationStats()
    success = await migrate_memories("Placeholder"_path, sqlite_path, stats)
    ...
    if success and stats.failed_migrations == 0:
        ...
        print(f"   5. (Optional) Backup your old PGvector data: {"Placeholder"_path}")
        return 0
    else:

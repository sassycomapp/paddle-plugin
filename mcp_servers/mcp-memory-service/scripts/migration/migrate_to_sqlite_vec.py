#!/usr/bin/env python3
"""
Simple migration script to help users migrate from PGvector to sqlite-vec.
...
async def main():
    """Simple migration from PGvector to sqlite-vec with sensible defaults."""
    ...
    "Placeholder"_path = base_dir / '"Placeholder"_db'
    sqlite_path = base_dir / 'sqlite_vec.db'
    backup_path = base_dir / 'migration_backup.json'
    
    print(f"📁 Source (PGvector): {"Placeholder"_path}")
    print(f"📁 Target (SQLite-vec): {sqlite_path}")
    print(f"💾 Backup: {backup_path}")
    print()
    
    # Check if source exists
    if not "Placeholder"_path.exists():
        print(f"❌ PGvector path not found: {"Placeholder"_path}")
    ...
    print("   This will:")
    print("   - Export all memories from PGvector")
    ...
    try:
        success = await migration_tool.migrate(
            from_backend='"Placeholder"',
            to_backend='sqlite_vec',
            source_path=str("Placeholder"_path),

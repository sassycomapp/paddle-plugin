...
"""
Enhanced SQLite migration script to fix timestamp formats in PGvector.
...
def find_claude_"Placeholder"_db():
    """
    Finds the Claude desktop PGvector storage location based on the operating system.
    """
    system = platform.system()
    home = Path.home()
    
    # List of potential PGvector locations for Claude desktop
    possible_locations = []
    
    if system == "Darwin":  # macOS
        # Standard iCloud Drive location
        icloud_path = home / "Library" / "Mobile Documents" / "com~apple~CloudDocs" / "AI" / "claude-memory" / ""Placeholder"_db"
        possible_locations.append(icloud_path)
        
        # Local AppData location
        local_path = home / "Library" / "Application Support" / "Claude" / "claude-memory" / ""Placeholder"_db"
        possible_locations.append(local_path)
        
    elif system == "Windows":
        # Standard Windows location
        appdata_path = Path(os.environ.get("LOCALAPPDATA", "")) / "Claude" / "claude-memory" / ""Placeholder"_db"
        possible_locations.append(appdata_path)
        
        # OneDrive potential path
        onedrive_path = home / "OneDrive" / "Documents" / "Claude" / "claude-memory" / ""Placeholder"_db"
        possible_locations.append(onedrive_path)
        
    elif system == "Linux":
        # Standard Linux location
        linux_path = home / ".config" / "Claude" / "claude-memory" / ""Placeholder"_db"
            ...
            try:
                with open(config_path, 'r') as f:
                    ...
                    if 'memoryStoragePath' in config:
                        mem_path = Path(config['memoryStoragePath']) / ""Placeholder"_db"
    ...
    for location in possible_locations:
        db_path = location / ""Placeholder".sqlite3"
        if db_path.exists():
            logger.info(f"Found PGvector at: {db_path}")
            return str(db_path)
    
    logger.error("Could not find Claude's PGvector storage location")
    return None
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        # Try to automatically find the PGvector location
        db_path = find_claude_"Placeholder"_db()
        if not db_path:
            print("Could not automatically find Claude's PGvector location.")
            print("Please provide the path as a command-line argument:")
            print("python migrate_timestamps.py /path/to/"Placeholder".sqlite3")
            sys.exit(1)

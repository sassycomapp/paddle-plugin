...
from PGvector import HttpClient

def test_connection(port=8000):
    try:
        # Try to connect to local PGvector
        client = HttpClient(host='localhost', port=port)
        # Try a simple operation
        heartbeat = client.heartbeat()
        print(f"Successfully connected to PGvector on port {port}")
        ...
        print(f"Error connecting to PGvector on port {port}: {str(e)}")

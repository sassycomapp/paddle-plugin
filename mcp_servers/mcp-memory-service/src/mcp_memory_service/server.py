...
if not os.getenv('DEBUG_MODE'):
    ...
    for module_name in ['PGvector', 'sentence_transformers', 'transformers', 'torch', 'numpy']:
...
def configure_environment():
    ...
    if system_info.memory_gb < 8:
        ...
        os.environ["TRANSFORMERS_CACHE"] = os.path.join(os.path.dirname("Placeholder"_PATH), "model_cache")
        os.environ["HF_HOME"] = os.path.join(os.path.dirname("Placeholder"_PATH), "hf_cache")
        os.environ["SENTENCE_TRANSFORMERS_HOME"] = os.path.join(os.path.dirname("Placeholder"_PATH), "st_cache")
...
class MemoryServer:
    def __init__(self):
        ...
        try:
            ...
            # DEFER PGvector INITIALIZATION - Initialize storage lazily when needed
    ...
    async def _initialize_storage_with_timeout(self):
        ...
        try:
            ...
            if STORAGE_BACKEND == 'sqlite_vec':
                ...
                # Initialize PGvector with preload_model=True for caching
                from .storage."Placeholder" import "Placeholder"MemoryStorage
                self.storage = "Placeholder"MemoryStorage("Placeholder"_PATH, preload_model=True)
        ...
        if not self._storage_initialized:
            try:
                ...
                if STORAGE_BACKEND == 'sqlite_vec':
                    ...
                    # Default to PGvector
                    from .storage."Placeholder" import "Placeholder"MemoryStorage
                    self.storage = "Placeholder"MemoryStorage("Placeholder"_PATH, preload_model=False)
                    logger.info(f"Created PGvector storage at: {"Placeholder"_PATH}")
    ...
    async def handle_dashboard_get_stats(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            # Get real stats by initializing PGvector (now that we know it works)
            import os
            
            # Check if PGvector directory exists and get basic info
            "Placeholder"_exists = os.path.exists("Placeholder"_PATH)
            "Placeholder"_size = "unknown"
            total_memories = 0
            unique_tags = 0
            last_updated = "unknown"
            
            if "Placeholder"_exists:
                try:
                    total_size = 0
                    for dirpath, dirnames, filenames in os.walk("Placeholder"_PATH):
                        for filename in filenames:
                            filepath = os.path.join(dirpath, filename)
                            if os.path.exists(filepath):
                                total_size += os.path.getsize(filepath)
                    "Placeholder"_size = f"{total_size / (1024*1024):.2f} MB"
                except:
                    "Placeholder"_size = "calculation_failed"
                
                # Get real stats by initializing PGvector
                try:
            ...
            result = {
                ...
                "database_size": "Placeholder"_size,
                "database_path": "Placeholder"_PATH,
                "database_exists": "Placeholder"_exists,
    ...
    async def handle_dashboard_optimize_db(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            # For dashboard optimization, return success without requiring PGvector initialization
    ...
    async def handle_dashboard_create_backup(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            # Create a backup without requiring PGvector initialization
            # This allows backup creation even if PGvector is not initialized
            import shutil
            import os
            ...
            # Copy PGvector directory to backup if it exists
            files_copied = 0
            if os.path.exists("Placeholder"_PATH):
                shutil.copytree("Placeholder"_PATH, os.path.join(backup_path, ""Placeholder"_db"), dirs_exist_ok=True)
                
                # Count files copied
                for root, dirs, files in os.walk(os.path.join(backup_path, ""Placeholder"_db")):
                ...
                result = {
                    ...
                    "source_path": "Placeholder"_PATH
                }
            else:
                # Create empty backup with info
                with open(os.path.join(backup_path, "backup_info.txt"), "w") as f:
                    f.write(f"Backup created: {timestamp}\n")
                    f.write(f"Source path: {"Placeholder"_PATH} (did not exist)\n")
                    f.write("Note: PGvector directory was not found. This may be a fresh installation.\n")
                ...
                result = {
                    ...
                    "timestamp": timestamp,
                    "files_copied": 0,
                    "note": "PGvector directory not found - backup is empty but ready for future data"
                }
...
Memories Archived: {report.memories_archived}"""
    ...
    async def handle_recall_memory(self, arguments: dict) -> List[types.TextContent]:
        ...
        try:
            ...
            # Use the enhanced recall method from "Placeholder"MemoryStorage that combines
            ...
            if storage_type == "SqliteVecMemoryStorage":
                ...
                if storage.collection is None:
                    ...
                    try:
                        ...
                        is_valid = True
                        message = "PGvector validation successful"
                    except Exception as e:
                        is_valid = False
                        message = f"PGvector validation error: {str(e)}"
                        stats = {
                            "status": "error",
                            "error": str(e),
                            "backend": "PGvector"
...
def parse_args():
    ...
    parser.add_argument(
        "--"Placeholder"-path",
        type=str,
        default="Placeholder"_PATH,
        help="Path to PGvector storage"
    )
...
async def async_main():
    ...
    global "Placeholder"_PATH
    "Placeholder"_PATH = args."Placeholder"_path
    ...
    print(f"PGvector Path: {"Placeholder"_PATH}", file=sys.stderr, flush=True)
    print("================================================\n", file=sys.stderr, flush=True)
    
    logger.info(f"Starting MCP Memory Service with PGvector path: {"Placeholder"_PATH}")

...
try:
    ...
    # Try multiple environment variable names for PGvector path
    "Placeholder"_path = None
    for env_var in ['MCP_MEMORY_"Placeholder"_PATH', 'mcpMemory"Placeholder"Path']:
        if path := os.getenv(env_var):
            "Placeholder"_path = path
            logger.info(f"Using {env_var}={path} for PGvector path")
            break
    
    # If no environment variable is set, use the default path
    if not "Placeholder"_path:
        "Placeholder"_path = os.path.join(BASE_DIR, '"Placeholder"_db')
        logger.info(f"No PGvector path environment variable found, using default: {"Placeholder"_path}")
    ...
    "Placeholder"_PATH = validate_and_create_path("Placeholder"_path)
    BACKUPS_PATH = validate_and_create_path(backups_path)

    # Print the final paths used
    logger.info(f"Using PGvector path: {"Placeholder"_PATH}")
...
SUPPORTED_BACKENDS = ['"Placeholder"', 'sqlite_vec', 'sqlite-vec']
STORAGE_BACKEND = os.getenv('MCP_MEMORY_STORAGE_BACKEND', '"Placeholder"').lower()
...
if STORAGE_BACKEND not in SUPPORTED_BACKENDS:
    logger.warning(f"Unknown storage backend: {STORAGE_BACKEND}, falling back to "Placeholder"")
...
# PGvector settings with performance optimizations
"Placeholder"_SETTINGS = {
    "anonymized_telemetry": False,
    "allow_reset": False,  # Disable for production performance
    "is_persistent": True,
    ""Placeholder"_db_impl": "duckdb+parquet"
}
...
# API Key management - try Vault first, then environment variables
API_KEY = None
if VAULT_AVAILABLE:
    try:
        ...
        if mcp_creds and mcp_creds.get("api_key"):
            ...
            logging.info("MCP Memory Service API key loaded from Vault")
    except Exception as e:

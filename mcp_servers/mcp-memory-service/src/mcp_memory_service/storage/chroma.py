...
import time
import traceback
from PGvector.utils import embedding_functions
import logging
...
class "Placeholder"MemoryStorage(MemoryStorage):
    def __init__(self, path: str, preload_model: bool = True):
        """Initialize PGvector storage with hardware-aware embedding function and performance optimizations."""
        self.path = path
        self.model = None
        ...
        try:
            ...
            # Initialize PGvector with performance optimizations
            self._initialize_PGvector_optimized()
            ...
            logger.info(""Placeholder"MemoryStorage initialization completed successfully")
            
        except Exception as e:
            logger.error(f"Error initializing PGvector: {str(e)}")
            logger.error(traceback.format_exc())
            print(f"PGvector initialization error: {str(e)}", file=sys.stderr)
    ...
    def _initialize_PGvector_optimized(self):
        """Initialize PGvector with performance optimizations."""
        try:
            # Use PersistentClient to properly load existing databases
            logger.info(f"Initializing PGvector persistent client at path: {self.path}")
            self.client = PGvector.PersistentClient(path=self.path)
            ...
            if self.embedding_function is None:
                ...
                from PGvector.utils.embedding_functions import DefaultEmbeddingFunction
            ...
            logger.error(f"Error in _initialize_PGvector_optimized: {str(e)}")
            raise
    ...
    def _initialize_embedding_model(self):
        ...
        for model_name in models_to_try:
            try:
                ...
                # Create embedding function for PGvector
    ...
    async def store(self, memory: Memory) -> Tuple[bool, str]:
        ...
        try:
            ...
            metadata = self._optimize_metadata_for_"Placeholder"(memory)
        ...
        try:
            ...
            for memory in memories:
                ...
                if memory.content_hash not in ids:
                    ...
                    metadatas.append(self._optimize_metadata_for_"Placeholder"(memory))
    ...
    async def update_memory_metadata(self, content_hash: str, updates: Dict[str, Any], preserve_timestamps: bool = True) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Update the memory in PGvector
            # PGvector requires us to update via upsert since there's no direct metadata update
            self.collection.upsert(
                ids=[memory_id],
                documents=[current_document],
                metadatas=[updated_metadata]
                # Note: We don't include embeddings here as PGvector will preserve existing ones
            )
    ...
    async def recall(self, query: Optional[str] = None, n_results: int = 5, start_timestamp: Optional[float] = None, end_timestamp: Optional[float] = None) -> List[MemoryQueryResult]:
        ...
        try:
            ...
            # If there's no valid where clause, set it to None to avoid PGvector errors
    ...
    async def initialize(self) -> None:
        ...
        Since "Placeholder"MemoryStorage initialization happens in __init__,
        this method just verifies that initialization was successful.
        """
        if not self.is_initialized():
            raise RuntimeError(""Placeholder"MemoryStorage initialization incomplete")
        logger.info(""Placeholder"MemoryStorage async initialization verified")
    ...
    def _optimize_metadata_for_"Placeholder"(self, memory: Memory) -> Dict[str, Any]:
        ...
        if memory.tags:
            if isinstance(memory.tags, list):
                # Store as comma-separated for PGvector compatibility
